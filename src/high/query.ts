...
const queryi = <T extends Node>(
  selector: string,
  contextNode?: Node | ParentNode
): Array<T> => {
  const context = contextNode || document;
  if (isXPath(selector)) {
    const result = document.evaluate(selector, context);
    if (
      result.resultType == XPathResult.ORDERED_NODE_ITERATOR_TYPE ||
    ...
    throw new Error(
      `unsupported result type ${result.resultType} by ${selector}`
    );
  } else {
    return Array.from(
      (context as ParentNode).querySelectorAll(selector)
    ) as unknown as Array<T>;
  }
};
queryi.context = (contextNode: Node | ParentNode) => (selector: string) =>
  queryi(selector, contextNode);
